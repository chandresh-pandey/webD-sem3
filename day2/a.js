// console.log("===Variables===");
// var text = "Chandresh";
// var last = "Pandey";
// console.log(`${text}  ${last}`);
// console.log(text);
// console.log(last);
// console.log("\n===if Else===");
// var a=40;
// var b=30;

// if(a>b){
//     console.log("A is greater, value of A =",a);
// }else{
//     console.log("B is greater, value of B =",b);
// }
// console.log("\n===Functions===");
// function abes(){
//     console.log("Welcome Students");
//     console.log("Welcome to Node.js");
// }
// abes();
// abes();
// console.log("\nFunction with Parameter");
// function greet(fname,lname){
//     console.log("Hello,",fname,lname);
// }
// greet("Chandresh","Pandey");
// greet("Chandresh","Pandey");

// console.log("\n===Function Return===");

// function add(a,b){
//     return a+b;
// }

// var result=add(20,30);
// console.log("Sum = ",result);
// console.log("\n===defult argument===");

// function fullname(fname="Abes",lname="Engineering College"){
//     return fname+" "+lname;
// }
// console.log(fullname("chandresh","pandey"));
// console.log(fullname());
console.log("\n===Percentage===");

function totalMarks(maths,engilsh,science){
    return maths+engilsh+science;
}
function percentage(total){
    return (total/300)*100;
}

var total = totalMarks(60,70,90);

console.log("Total Marks:",total);
console.log("Percentage",percentage(total)+"%");